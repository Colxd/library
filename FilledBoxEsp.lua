...
get("player").new = function(self, player)
	local function cache(character)
		self.cache[character] = { ["player"] = player, ["drawings"] = { ["boxFilled"] = get("new").drawing("Square", { Visible = false, Filled = true }) } }
	end
...
get("player").update = function(self, character, data)
	...
	task.spawn(function()
		...
		if visible and check() then
			...
			drawings.boxFilled.Position = Vector2.new(xPosition, yPostion)
		...
		if drawings.boxFilled then drawings.boxFilled.Visible = (check() and visible and visuals.boxes.filled.enabled) end
	end)
end
...
declare(features, "visuals", {
	...
	["boxes"] = { ["filled"] = { ["enabled"] = true, ["color"] = Color3.fromRGB(255, 255, 255), ["transparency"] = 0.25 } }
})
