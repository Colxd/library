-- Health ESP Library by Blissful#4992
...
get("player").new = function(self, player)
	local function cache(character)
		self.cache[character] = { ["player"] = player, ["drawings"] = { ["health"] = get("new").drawing("Line", { Visible = false }), ["healthOutline"] = get("new").drawing("Line", { Visible = false }), ["healthText"] = get("new").drawing("Text", { Visible = false, Center = false, Font = Drawing.Fonts.Plex}) } }
	end
...
get("player").update = function(self, character, data)
	...
	local player = data.player; local root = character.HumanoidRootPart; local humanoid = character.Humanoid; local drawings = data.drawings
